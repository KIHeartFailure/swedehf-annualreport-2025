...
The information presented is based on data reported in SwedeHF until 14th of January 2026. 

The presentation is based on the date of the patients visit/admission.

Index is defined as the first time a patient is registered in SwedeHF, not necessarily with newly diagnosed heart failure. For the follow-up at 1-180 days after index the mean time from index was `r formatC(mean(rsdata %>% filter(ttype == "1-80 days follow-up", indexyear == global_year) %>% pull(diff_timeadmission), na.rm = T) / 30.5, format = "f", digits = 1, decimal.mark = ".")` months during `r global_year`. 
