...
The information presented is based on data reported in SwedeHF until 24th November 2025 ÄNDRA TILL 14th of January 2026. 

The presentation is based on the date of the patients visit/admission.

Index is defined as the first time a patient is registered in SwedeHF, not necessarily with newly diagnosed heart failure. For the follow-up at 1-180 days after index the mean time from index was `r formatC(mean(rsdata %>% filter(ttype == "1-80 days follow-up", indexyear == global_year) %>% pull(diff_timeadmission), na.rm = T) / 30.5, format = "f", digits = 1, decimal.mark = ".")` during `r global_year`. 
...
https://github.com/KIHeartFailure/swedehf-annualreport-2025. 
...
:::: {#fig-desk layout-ncol=2 layout-nrow=2}
:::{#fig-desk1}
```{r}
...
LVEF
:::
`{r} paste0("Patient characteristics at index, ", global_year)`
::::

:::: {#fig-desk layout-ncol=4 layout-nrow=1}
:::{#fig-desk1}
```{r}
...
:::{#fig-desk2}
```{r}
piefunc(var = "vtype", type = c("1-80 days follow-up"))
```
1-80 days follow-up
:::
:::{#fig-desk3}
...
2+ year follow-up
:::
`{r} paste0("Centre type, ", global_year)`
::::

:::: {#fig-desk layout-ncol=2 layout-nrow=1}
:::{#fig-desk1}
```{r}
#| fig-height: 8
#| fig-width: 7
...
#| fig-height: 8
#| fig-width: 7
barfunc(var = "vtype", type = c("Index", "1-80 days follow-up", "1 year follow-up", "2+ year follow-up"))
```
All registrations
:::
`{r} paste0("Centre type by region, ", global_year)`
::::
...
qinametmp <- qiinfo %>%
  filter(qivar == qitmp) %>%
  pull(qiname)
startimetmp <- global_startdtm
stoptimetmp <- global_stopdtm
startyeartmp <- global_year - 7
```             
...
qinametmp <- qiinfo %>%
  filter(qivar == qitmp) %>%
  pull(qiname)
startimetmp <- ymd(paste0(global_year - 1, "-07-01 - "))
stoptimetmp <- ymd(paste0(global_year, "-07-01"))
startyeartmp <- global_year - 7
```             

## Quality Indicator `r paste0(qiinfo %>% filter(qivar == qitmp) %>% pull(qino), ". ", firstcap(qinametmp))` 

**Numerator:** Patients who have had a follow-up visit at 1-180 days after index performed and documented.
...
*The quality indicator is defined at the 1-80 days follow-up.*

{{< include src/qifu.qmd >}}
...
**Numerator:** Patients with LVEF < 40% (prior to October 2025)/LVEF $\le$ 40% (from October 2025), who are alive at discharge and 
where ACEI/ARB/ARNI, beta-blockers, MRA and SGLT2 inhibitors have been prescribed.

**Denominator:** Patients where LVEF < 40% (prior to October 2025)/LVEF $\le$ 40% (from October 2025), who are alive at discharge and 
...
**Denominator:** Patients with LVEF < 40% (prior to October 2025)/LVEF $\le$ 40% (from October 2025), who are alive at discharge and 
...
**Denominator:** Patients where LVEF < 40% (prior to October 2025)/LVEF $\le$ 40% (from October 2025), who are alive at discharge 
...
**Denominator:** Patients where LVEF < 40% (prior to October 2025)/LVEF $\le$ 40% (from October 2025), who are alive at discharge and where 
...
**Numerator:** Patients with LVEF 40% (prior to October 2025)/41% (from October 2025)-49%, who are alive at discharge and 
where SGLT2 inhibitors have been prescribed.

**Denominator:** Patients where LVEF 40% (prior to October 2025)/41% (from October 2025)-49%, who are alive at discharge and where 
information on the prescription of SGLT2 inhibitors has been documented. If LVEF is missing, last observation is carried forward. 
...
or < 40% (prior to October 2025)/$\le$ 40% (from October 2025) (semiquantitative LVEF documented) and with a CRT (with or without ICD) implanted.

**Denominator:** Patients with LVEF $\leq$ 35% (LVEF as a percentage documented) 
or < 40% (prior to October 2025)/$\le$ 40% (from October 2025) (semiquantitative LVEF documented) and where information about device 
...
**Numerator:** Patients with LVEF $\leq$ 35% (LVEF as a percentage documented) 
or < 40% (prior to October 2025)/$\le$ 40% (from October 2025) (semiquantitative LVEF documented) and with a ICD (with or without CRT) implanted. 

**Denominator:** Patients with LVEF $\leq$ 35% (LVEF as a percentage documented) or < 40% (prior to October 2025)/$\le$ 40% (from October 2025) (semiquantitative LVEF documented) and where information about device has been documented. If LVEF is missing, last observation is carried forward. 
