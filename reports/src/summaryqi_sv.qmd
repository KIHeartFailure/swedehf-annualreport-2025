...
#| fig-cap: !expr paste0("Riket, alla ", nrow(qiinfo) -3, " kvalitetsindikatorer ",  global_year)
#| fig-height: 6
#| fig-width: 10
#| out-width: "125%"
#| out-height: "125%"
...
stat2$qishortname_sv <- forcats::fct_reorder(stat2$qishortname_sv, stat2$row)
...
legend(33, 102.5, levhf, fill = global_cols[c(1, 2)], border = global_cols[c(1, 2)], bty = "n", cex = cexmy)
...
axis(1,
  at = bplace + 0.5,
  labels = setdiff(qiinfo$qishortname_sv, c("SGLT2i (HFrEF)", "SGLT2i (HFmrEF)", "SGLT2i (HFpEF)")),
...
axis(1,
  at = b[22] + (b[23] - b[22]) / 2,
  labels = "Uppföljning 3 månader", line = 6.7, tick = FALSE, cex.axis = cexmy, gap.axis = -10000000, las = 1
)
...
arrows(b[11] + adjmin, posarrow, b[19], posarrow, xpd = TRUE, code = 1, length = 0.1)
arrows(b[26], posarrow, b[30] - adjmin, posarrow, xpd = TRUE, code = 2, length = 0.1)

legend("bottom",
  inset = c(-0, -.67), xpd = NA,
  legend = rev(global_labnamssv[2:3]),
  lty = 2,
  lwd = 1,
