...
tabvars_nosex <- setdiff(tabvars, "sex")
...
`{r} paste0("Patient characteristics, ", global_year)`

::::

:::: {#tbl-tabchar-sex tbl-pos="H"}
...
for (i in seq_along(tabvars_nosex)) {
  tabtmp_fem <- basetabfunc(tabvars_nosex[i],
    data = yearrsdata %>% filter(sex == "Female"), stratavar = "ttype",
    catdig = catdig, contdig = contdig
  )
  tabtmp_mal <- basetabfunc(tabvars_nosex[i],
    data = yearrsdata %>% filter(sex == "Male"), stratavar = "ttype",
    catdig = catdig, contdig = contdig
  )
...
`{r} paste0("Patient characteristics by sex, ", global_year)`
