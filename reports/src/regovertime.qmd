...
p <- all %>%
  ...
  scale_y_continuous(breaks = seq(0, 22000, 2000), limits = c(0, 22000), labels = comma_format(), expand = c(0.05, 0.05)) +
  labs(y = "Number of registrations", x = "Year")

p
```

`{r} paste0("Registration over time, ", global_year - 7, "-", global_year)`

::::

:::: {#tbl-regovertime tbl-pos="H"}

```{r}
#| dependson: regovertime
