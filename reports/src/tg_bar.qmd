...
:::: {#fig-tg layout-ncol=2 layout-nrow=2}
::: {#fig-tg1}
```{r}
#| fig-height: 8
#| fig-width: 7
...
```  
`{r} paste0("Prevalent (map), ", global_year)`
:::
...
ggplot(mapfig %>% filter(typetg == "Incident"), aes(fill = p_cat)) +
  ...
  scale_fill_manual(values = c(global_cols, "lightgrey"))
```  
`{r} paste0("Incident (map), ", global_year - 1)`
:::
::: {#fig-tg3}
```{r}
#| fig-height: 8
...
```  
`{r} paste0("Prevalent, ", global_year)`
:::
...
```  
`{r} paste0("Incident, ", global_year - 1)`
:::
Coverage by county
::::
